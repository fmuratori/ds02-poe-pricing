...
INV_CURRENCY_DICT = {value: k for k, values in CURRENCY_DICT.items() for value in values }
...
def extract_item_price(items, use_stash_price=False):
    ...
    (items['price_currency'], items['price_quantity'], items['sell_quantity'], items['is_stash_price']) = zip(*prices)
    return items
...
def filter_item_category(items, categories):
    if 'jewels' in categories:
        items = items[~((items.category=='jewels') & (items.sub_category=='cluster'))]
...
def filter_item_price(items):
    items['price_currency'] = items.price_currency.apply(lambda y: y.lower() if y is not None else None)
