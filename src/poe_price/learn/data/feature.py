...
def is_abyss_jewel(data):
    return data['items'].sub_category.apply(lambda y: 0 if y is None or y != 'abyss' else 1).values.reshape(-1,1)
        ...
        for i, wid in enumerate(data['items'].id.values):
            for pid, mod in data['items_modifiers'] \
                ...
                if mod.modifier_id in self.modifiers_lut:
                    ...
                    values = [v for v in [mod.value0, mod.value1, mod.value2] if v is not None and not np.isnan(v)]
        ...
        for i, wid in enumerate(data['items'].id.values):
            for pid, prop in data['items_properties'] \
                ...
                if prop.property_id in self.properties_lut:
                    ...
                    values = [v for v in [prop.value0, prop.value1] if v is not None]
...
class Rarity:
    ...
    def fit_transform(self, data):
        self.ohe = OneHotEncoder()
        return self.ohe.fit_transform(data['items'].rarity.values.reshape(-1,1))

    def transform(self, data):
        return self.ohe.transform(data['items'].rarity.values.reshape(-1,1))
